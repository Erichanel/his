...
def main():
    ...
    parser.add_argument("--config", default="configs/classify_hsivq_ssse.yaml")
    ...
    if data is not None and data.gt is not None:
        ...
        y = (y - 1).long().to(device, non_blocking=True)
    ...
    lambda_mtp = _get(cfg, "finetune", "lambda_mtp", default=0.0)
    out = model.forward_classify(x, y=y, lambda_mtp=lambda_mtp)
    loss = out["loss_total"]
    loss.backward()
    ...
    print("logits_cls", tuple(out["logits_cls"].shape))
